...
async function init(e){const q=db(e);
 ...
 await q`CREATE TABLE IF NOT EXISTS settings(id INTEGER PRIMARY KEY DEFAULT 1,background_url TEXT DEFAULT '',search_engine TEXT DEFAULT 'https://www.bing.com/search?q=',theme VARCHAR(30) DEFAULT 'glass')`;
...
async function api(r,e){try{await init(e);const q=db(e),u=new URL(r.url),p=u.pathname,m=r.method;
 if(p==="/api/data"&&m==="GET"){const [categories,sites,settings]=await Promise.all([q`SELECT * FROM categories ORDER BY sort_order,id`,q`SELECT * FROM sites ORDER BY category_id NULLS LAST,sort_order,id`,q`SELECT * FROM settings WHERE id=1`]);return j({categories,sites,settings:settings[0]||null})}
