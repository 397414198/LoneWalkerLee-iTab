...
const WidgetRegistry={
  ...
  calendar:{title:"超级日历",icon:"📅",sizes:[[1,2],[2,2],[2,3]],desc:"公历、农历、节假日、周数与年度信息"},
  work:{title:"下班倒计时",icon:"💰",sizes:[[1,1],[2,1],[2,2]],desc:"上下班时间、日薪、今日已赚"},
  todo:{title:"待办事项",icon:"✅",sizes:[[1,1],[2,1],[2,2]],desc:"待办、完成状态、进度统计与云端同步"},
  memo:{title:"云端便签",icon:"📝",sizes:[[1,1],[2,1],[2,2]],desc:"随手记录，保存到 Neon，电脑手机同步"},
  pomodoro:{title:"番茄钟",icon:"🍅",sizes:[[1,1],[2,1]],desc:"25 分钟专注 / 5 分钟休息"},
  countdown:{title:"倒计时",icon:"⏳",sizes:[[1,1],[2,1]],desc:"自定义日期与事件倒计时"},
...
function renderFolderBar(){let bar=$("#folderBar");if(!bar)return;let all=S.cat==null?S.sites:S.sites.filter(x=>x.category_id===S.cat);let h=`<button class="folderChip ${S.folder==null?'active':''}" onclick="filterFolder(null)">🗂️ 全部网站</button>`;h+=S.folders.map(f=>{let n=all.filter(x=>x.folder_id===f.id).length;return `<button class="folderChip ${S.folder===f.id?'active':''}" onclick="filterFolder(${f.id})">📁 ${esc(f.name)} <span class="folderCount">${n}</span></button>`}).join("");bar.innerHTML=h+`<span class="folderHint">点击文件夹筛选网站</span>`}
...
function renderSites(){let a=S.cat==null?S.sites:S.sites.filter(x=>x.category_id===S.cat);if(S.folder!=null)a=a.filter(x=>x.folder_id===S.folder);let folder=S.folders.find(f=>f.id===S.folder);$("#sites").innerHTML=a.length?a.map(s=>`<a class="site" href="${esc(s.url)}" target="_blank" rel="noopener"><div class="tools"><button onclick="event.preventDefault();edit(${s.id})">✎</button><button onclick="event.preventDefault();del(${s.id})">×</button></div><div class="ico">${iconHtml(s)}</div><div class="name">${esc(s.title)}</div><div class="link">${esc(s.url)}</div>${s.folder_id?`<div class="siteFolder">📁 ${esc(S.folders.find(f=>f.id===s.folder_id)?.name||"文件夹")}</div>`:""}</a>`).join(""):`<div style="grid-column:1/-1;text-align:center;opacity:.55;padding:35px">${folder?`文件夹「${esc(folder.name)}」里还没有网站。`:'这里还没有网站，点“添加网站”吧 😎'}</div>`}
function filterFolder(id){S.folder=id;renderFolderBar();renderSites()}
window.filter=x=>{S.cat=x;cats();renderSites()};
window.edit=id=>{let s=S.sites.find(x=>x.id===id);$("#dt").textContent="编辑网站";$("#sid").value=s.id;$("#title").value=s.title;$("#url").value=s.url;$("#icon").value=s.icon||"🔗";$("#category").value=s.category_id||"";$("#folder").value=s.folder_id||"";$("#autoIcon").checked=!isIconUrl(s.icon);updateIconPreview();dlg.showModal()};
...
function dayOfYear(d){let a=new Date(d.getFullYear(),0,1);return Math.floor((d-a)/86400000)+1}
function weekNo(d){let x=new Date(Date.UTC(d.getFullYear(),d.getMonth(),d.getDate()));let day=x.getUTCDay()||7;x.setUTCDate(x.getUTCDate()+4-day);let y=new Date(Date.UTC(x.getUTCFullYear(),0,1));return Math.ceil((((x-y)/86400000)+1)/7)}
function holidayLabel(info){if(!info)return "";return info.name||info.holiday||info.desc||"节假日"}
async function renderCalendarWidget(el,w){let c=Object.assign({showLunar:true,showHoliday:true,showStats:true},w.config||{}),base=new Date(),offset=Number(c.monthOffset||0),view=new Date(base.getFullYear(),base.getMonth()+offset,1),y=view.getFullYear(),m=view.getMonth(),first=new Date(y,m,1).getDay(),last=new Date(y,m+1,0).getDate(),names=["日","一","二","三","四","五","六"],hol=c.showHoliday!==false?await holidayMonth(y,m+1):{};let h=`<div class="calTop"><button class="miniBtn" onclick="calendarMove(${w.id},-1)">‹</button><div class="calTitle"><b>${y} 年 ${m+1} 月</b>${offset===0?'<small>本月</small>':''}</div><button class="miniBtn" onclick="calendarMove(${w.id},1)">›</button></div><div class="calQuick"><button class="miniBtn" onclick="calendarToday(${w.id})">今天</button><span>${m+1} 月 · ${last} 天</span></div><div class="calendarGrid">${names.map(x=>`<div class="calHead">${x}</div>`).join("")}`;for(let i=0;i<first;i++)h+="<div></div>";for(let n=1;n<=last;n++){let dt=new Date(y,m,n),iso=localISO(dt),hinfo=hol[iso],cls=(offset===0&&n===base.getDate())?"today":"",holiday=holidayLabel(hinfo),isWeekend=dt.getDay()===0||dt.getDay()===6;h+=`<div class="calCell ${cls} ${isWeekend?'weekend':''}" title="${esc(holiday)}"><b>${n}</b>${c.showLunar!==false?`<small>${esc(lunarText(dt).replace(/^.*?(正月|二月|三月|四月|五月|六月|七月|八月|九月|十月|冬月|腊月)/,""))}</small>`:""}${holiday?`<em>${esc(holiday)}</em>`:isWeekend?'<em>周末</em>':''}</div>`}h+="</div>";let todayInfo=hol[localISO(base)],stats=c.showStats!==false?`<div class="calStats"><span>📅 第 ${dayOfYear(base)} 天</span><span>🗓️ 第 ${weekNo(base)} 周</span><span>🌙 ${esc(lunarText(base))}</span></div>`:"";let holidayCount=Object.keys(hol).length;el.querySelector(".widget-body").innerHTML=h+stats+`<div class="lunarToday">今天：${esc(lunarText(base))}${todayInfo?" · "+esc(holidayLabel(todayInfo)):""}${c.showHoliday!==false?` · 本月标记 ${holidayCount} 天`:""}</div>`}
...
window.calendarToday=async id=>{let w=S.widgets.find(x=>x.id===id);if(!w)return;w.config=w.config||{};w.config.monthOffset=0;let el=document.querySelector(`.widget[data-id="${id}"]`);if(el)await renderCalendarWidget(el,w)};
window.calendarMove=async(id,delta)=>{let w=S.widgets.find(x=>x.id===id);if(!w)return;w.config=w.config||{};w.config.monthOffset=(Number(w.config.monthOffset)||0)+delta;let el=document.querySelector(`.widget[data-id="${id}"]`);if(el)await renderCalendarWidget(el,w)};
...
function renderTodoWidget(el,w){
  let c=Object.assign({items:[],filter:"all"},w.config||{}),items=Array.isArray(c.items)?c.items:[];
  let visible=items.map((x,i)=>({...x,_i:i})).filter(x=>c.filter==="active"?!x.done:c.filter==="done"?x.done:true);
  let done=items.filter(x=>x.done).length,total=items.length;
  let list=visible.map(x=>`<div class="todoItem ${x.done?'done':''}"><button class="todoCheck" onclick="toggleTodo(${w.id},${x._i})">${x.done?'✓':'○'}</button><span>${esc(x.text||'未命名')}</span><button class="todoDel" onclick="deleteTodo(${w.id},${x._i})">×</button></div>`).join('');
  el.querySelector('.widget-body').innerHTML=`<div class="todoSummary"><span>${done}/${total} 完成</span><div class="todoProgress"><i style="width:${total?Math.round(done/total*100):0}%"></i></div></div><div class="todoFilters"><button class="${c.filter==='all'?'on':''}" onclick="setTodoFilter(${w.id},'all')">全部</button><button class="${c.filter==='active'?'on':''}" onclick="setTodoFilter(${w.id},'active')">未完成</button><button class="${c.filter==='done'?'on':''}" onclick="setTodoFilter(${w.id},'done')">已完成</button>${done?`<button class="todoClear" onclick="clearDoneTodo(${w.id})">清理已完成</button>`:''}</div><div class="todoList">${list||'<div class="empty">没有符合条件的待办</div>'}</div><div class="todoAdd"><input id="todoInput${w.id}" placeholder="添加一件小事…" onkeydown="if(event.key==='Enter')addTodo(${w.id})"><button class="miniBtn" onclick="addTodo(${w.id})">＋</button></div>`;
}

function renderMemoWidget(el,w){
  let c=Object.assign({text:"",updatedAt:""},w.config||{});
  el.querySelector('.widget-body').innerHTML=`<textarea class="memoArea" id="memoArea${w.id}" placeholder="写点什么……支持电脑、手机云端同步">${esc(c.text)}</textarea><div class="memoFoot"><span>${c.text?`上次保存：${esc(new Date(c.updatedAt||Date.now()).toLocaleString('zh-CN'))}`:'还没有保存内容'}</span><button class="miniBtn" onclick="saveMemo(${w.id})">☁️ 保存</button></div>`;
}

async function updateTodo(w,items){w.config={items};await saveWidget(w);renderWidgets()}
window.addTodo=async id=>{if(!requireAdmin())return;let w=S.widgets.find(x=>x.id===id);if(!w)return;let input=$(`#todoInput${id}`),text=input.value.trim();if(!text)return;let items=Array.isArray(w.config?.items)?w.config.items.slice():[];items.push({text,done:false});await updateTodo(w,items)};
window.toggleTodo=async(id,i)=>{if(!requireAdmin())return;let w=S.widgets.find(x=>x.id===id);if(!w)return;let items=Array.isArray(w.config?.items)?w.config.items.slice():[];if(!items[i])return;items[i]={...items[i],done:!items[i].done};await updateTodo(w,items)};
window.deleteTodo=async(id,i)=>{if(!requireAdmin())return;let w=S.widgets.find(x=>x.id===id);if(!w)return;let items=Array.isArray(w.config?.items)?w.config.items.slice():[];items.splice(i,1);await updateTodo(w,items)};
window.setTodoFilter=async(id,filter)=>{let w=S.widgets.find(x=>x.id===id);if(!w)return;w.config=Object.assign({},w.config,{filter});saveLocal();let el=document.querySelector(`.widget[data-id="${id}"]`);if(el)renderTodoWidget(el,w)};
window.clearDoneTodo=async id=>{if(!requireAdmin())return;let w=S.widgets.find(x=>x.id===id);if(!w)return;let items=(Array.isArray(w.config?.items)?w.config.items:[]).filter(x=>!x.done);await updateTodo(w,items)};
window.saveMemo=async id=>{if(!requireAdmin())return;let w=S.widgets.find(x=>x.id===id);if(!w)return;let el=document.querySelector(`#memoArea${id}`),text=el?.value||'';w.config={text,updatedAt:new Date().toISOString()};await saveWidget(w,false);let card=document.querySelector(`.widget[data-id="${id}"]`);if(card)renderMemoWidget(card,w)};


function renderPomodoroWidget(el,w){
  let c=Object.assign({mode:'focus',seconds:1500,running:false},w.config||{}); let sec=Math.max(0,Number(c.seconds)||0); let mm=String(Math.floor(sec/60)).padStart(2,'0'),ss=String(sec%60).padStart(2,'0');
  el.querySelector('.widget-body').innerHTML=`<div class="pomoMode">${c.mode==='focus'?'专注时间':'休息时间'}</div><div class="pomoTime">${mm}:${ss}</div><div class="pomoBtns"><button class="miniBtn" onclick="pomoToggle(${w.id})">${c.running?'⏸ 暂停':'▶ 开始'}</button><button class="miniBtn" onclick="pomoReset(${w.id})">↺ 重置</button></div>`;
}
async function savePomo(w){await saveWidget(w,false);saveLocal();let el=document.querySelector(`.widget[data-id="${w.id}"]`);if(el)renderPomodoroWidget(el,w)}
window.pomoToggle=async id=>{if(!requireAdmin())return;let w=S.widgets.find(x=>x.id===id);if(!w)return;w.config=Object.assign({mode:'focus',seconds:1500,running:false},w.config||{});w.config.running=!w.config.running;await savePomo(w)};
window.pomoReset=async id=>{if(!requireAdmin())return;let w=S.widgets.find(x=>x.id===id);if(!w)return;w.config=Object.assign({},w.config,{mode:'focus',seconds:1500,running:false});await savePomo(w)};

function renderCountdownWidget(el,w){let c=Object.assign({title:'重要日子',date:''},w.config||{}),target=c.date?new Date(c.date):null,diff=target?target.getTime()-Date.now():NaN;
  let text=Number.isFinite(diff)?(diff<=0?'🎉 已到时间！':`还有 <b>${Math.ceil(diff/86400000)}</b> 天`):'还没有设置日期';
  el.querySelector('.widget-body').innerHTML=`<div class="countTitle">${esc(c.title||'重要日子')}</div><div class="countBig">${text}</div><div class="muted">${c.date?esc(new Date(c.date).toLocaleString('zh-CN')):'点击 ⚙️ 设置日期'}</div>`;
}
function renderWorldClockWidget(el,w){let zones=[['北京','Asia/Shanghai'],['东京','Asia/Tokyo'],['纽约','America/New_York'],['伦敦','Europe/London']];el.querySelector('.widget-body').innerHTML=`<div class="worldGrid">${zones.map(([n,z])=>{let d=new Date(),t=new Intl.DateTimeFormat('zh-CN',{timeZone:z,hour:'2-digit',minute:'2-digit',hour12:false}).format(d);return `<div><span>${n}</span><b>${t}</b></div>`}).join('')}</div>`}
...
function setNetStatus(offline=false){let el=$("#netStatus");if(!el)return;if(offline){el.textContent="📦 离线缓存";el.classList.add("offline")}else{el.textContent="☁️ 在线";el.classList.remove("offline")}}
function cacheAge(){try{let d=JSON.parse(localStorage.getItem(APP_CACHE_KEY)||"null");if(!d?.savedAt)return "";let t=Math.max(0,Date.now()-new Date(d.savedAt).getTime()),m=Math.floor(t/60000);return m<1?"刚刚更新":m<60?`${m} 分钟前更新`:`${Math.floor(m/60)} 小时前更新`}catch{return ""}}
...
else if(w.widget_key==="work")h=`<label>上班时间<input id="workStart" type="time" value="${esc(c.workStart||"08:30")}"></label><label>下班时间<input id="workEnd" type="time" value="${esc(c.workEnd||"17:30")}"></label><label>日薪（元）<input id="dailySalary" type="number" min="0" step="0.01" value="${esc(c.dailySalary??300)}"></label>`;
else if(w.widget_key==="countdown")h=`<label>事件名称<input id="countTitle" value="${esc(c.title||"重要日子")}"></label><label>目标日期<input id="countDate" type="datetime-local" value="${esc(c.date||"")}"></label>`;
else if(w.widget_key==="payday")h=`<label>每月发薪日<input id="payDay" type="number" min="1" max="28" value="${esc(c.day??10)}"></label><p class="muted">为避免月底月份天数差异，这里限制 1～28 日。</p>`;
else if(w.widget_key==="anniversary")h=`<label>纪念日名称<input id="annTitle" value="${esc(c.title||"我的纪念日")}"></label><label>开始日期<input id="annDate" type="date" value="${esc(c.date||"")}"></label>`;
...
function renderIconPalette(){const box=$("#iconPalette");if(!box)return;box.innerHTML=ICON_PRESETS.map(x=>`<button type="button" class="iconPick" data-icon="${esc(x)}">${x}</button>`).join("");box.querySelectorAll(".iconPick").forEach(b=>b.onclick=()=>{let id=+b.dataset.siteId;if(id)applySiteIcon(id,b.dataset.icon)});}
function iconCenterList(){let q=(($("#iconSearch")?.value)||"").trim().toLowerCase();let a=S.sites.filter(s=>!q||String(s.title).toLowerCase().includes(q)||String(s.url).toLowerCase().includes(q));let box=$("#iconSiteList");if(!box)return;box.innerHTML=a.length?a.map(s=>`<div class="iconSiteRow" data-id="${s.id}"><div class="iconSitePreview">${iconHtml(s)}</div><div class="iconSiteMeta"><b>${esc(s.title)}</b><small>${esc(s.url)}</small></div><div class="iconSiteBtns"><button type="button" data-action="auto">🌐 自动</button><button type="button" data-action="emoji">😊 Emoji</button><button type="button" data-action="url">🖼️ 图片</button></div></div>`).join(""):`<div class="folderEmpty">没有找到网站。</div>`;box.querySelectorAll('.iconSiteRow').forEach(row=>{let id=+row.dataset.id;row.querySelector('[data-action="auto"]').onclick=()=>applySiteIcon(id,faviconUrl(S.sites.find(x=>x.id===id)?.url||""));row.querySelector('[data-action="emoji"]').onclick=()=>{let icon=prompt("输入一个 Emoji，例如 🤖、☁️、🎬",S.sites.find(x=>x.id===id)?.icon||"🔗");if(icon)applySiteIcon(id,icon.trim())};row.querySelector('[data-action="url"]').onclick=()=>{let url=prompt("输入图片 URL（https://…）",isIconUrl(S.sites.find(x=>x.id===id)?.icon)?S.sites.find(x=>x.id===id).icon:"");if(url&&/^https?:\/\//i.test(url.trim()))applySiteIcon(id,url.trim());};});}
async function applySiteIcon(id,icon){if(!requireAdmin()||!icon)return;let s=S.sites.find(x=>x.id===id);if(!s)return;try{let saved=await api("/api/sites/"+id,{method:"PUT",body:JSON.stringify({...s,icon})});Object.assign(s,saved);renderSites();iconCenterList();saveLocal()}catch(e){alert(e.message)}}
async function autoAllCenterIcons(){if(!requireAdmin())return;let list=S.sites.filter(s=>faviconUrl(s.url));if(!list.length)return alert("没有可处理的网站。");if(!confirm(`将为 ${list.length} 个网站设置自动 favicon。已有自定义图标也会被替换。\n继续吗？`))return;let ok=0;for(const s of list){try{let saved=await api("/api/sites/"+s.id,{method:"PUT",body:JSON.stringify({...s,icon:faviconUrl(s.url)})});Object.assign(s,saved);ok++}catch(e){console.warn(e)}}renderSites();iconCenterList();saveLocal();alert(`完成：${ok}/${list.length}`)}
window.openIconCenter=()=>{if(!requireAdmin())return;renderIconPalette();iconCenterList();iconCenterDlg.showModal();};
$("#iconCenterBtn").onclick=openIconCenter;$("#iconSearch").oninput=iconCenterList;$("#iconAutoAll").onclick=autoAllCenterIcons;
...
$("#add").onclick=()=>{if(!requireAdmin())return;$("#dt").textContent="添加网站";$("#sid").value="";$("#title").value="";$("#url").value="";$("#icon").value="";$("#autoIcon").checked=true;$("#folder").value="";updateIconPreview();dlg.showModal()};
$("#icon").oninput=()=>{$("#autoIcon").checked=false;updateIconPreview()};
$("#url").oninput=()=>{if($("#autoIcon").checked)updateIconPreview()};
$("#siteForm").onsubmit=async e=>{e.preventDefault();try{let id=$("#sid").value,b={title:$("#title").value.trim(),url:$("#url").value.trim(),icon:$("#icon").value.trim(),category_id:Number($("#category").value),folder_id:$("#folder").value?Number($("#folder").value):null};if(!/^https?:\/\//i.test(b.url))b.url="https://"+b.url;if($("#autoIcon").checked)b.icon=faviconUrl(b.url)||"🔗";if(!b.icon)b.icon="🔗";await api(id?"/api/sites/"+id:"/api/sites",{method:id?"PUT":"POST",body:JSON.stringify(b)});dlg.close();load()}catch(x){alert(x.message)}};
window.openFolders=async()=>{if(!requireAdmin())return;renderFolderList();folderDlg.showModal()};
function renderFolderList(){let box=$("#folderList");if(!S.folders.length){box.innerHTML='<div class="folderEmpty">还没有文件夹，先创建一个吧 😎</div>';return}box.innerHTML=S.folders.map(f=>{let n=S.sites.filter(s=>s.folder_id===f.id).length;return `<div class="folderRow"><div class="folderMeta">📁 <b>${esc(f.name)}</b><small>${n} 个网站</small></div><button type="button" onclick="deleteFolder(${f.id})">删除</button></div>`}).join("")}
...
window.addEventListener("online",()=>{setNetStatus(false);load()});
